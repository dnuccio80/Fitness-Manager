...
import com.danucdev.fitnessmanager.ui.navigation.NavRoutes.ProductServices
                ...
                NavDisplay(
                    ...
                    entryProvider = entryProvider {
                        ...
                        entry<Clients> {
                            ClientsScreen(
                                currentRoute = currentRoute,
                                onBottomBarClick = { target -> backStack.backTo(target) }) { clientId ->
                        ...
                        entry<Config> {
                            SettingsScreen(onNavigateToProductServices = {
                                backStack.navigateTo(ProductServices)
                            }) { backStack.back() }
                        }
                        entry<ProductServices> {
                            ProductServicesScreen {
                                backStack.back()
                            }
                        }
                        entry<Transactions> {
                            TransactionsScreen(currentRoute) { target ->
                                backStack.backTo(target)
                            }
                        }
                    },
                    transitionSpec = {
